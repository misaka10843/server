...
pub async fn create(
    ...
    if let Some(credits) = data.credits {
        ...
        song_credit::Entity::insert_many(a).exec(tx).await?;

        song_credit_history::Entity::insert_many(b).exec(tx).await?;
    }

    if let Some(languages) = data.languages {
        let (a, b): VecPair<_, _> = languages
            .iter()
            .map(|language_id| {
                (
                    song_language::Model {
        ...
        song_language::Entity::insert_many(a).exec(tx).await?;
        song_language_history::Entity::insert_many(b)
            .exec(tx)
            .await?;
    }

    if let Some(localized_titles) = data.localized_titles {
        let (a, b): VecPair<_, _> = localized_titles
            .iter()
            .map(|localized_title| {
                (
        ...
        song_localized_title::Entity::insert_many(a)
            .exec(tx)
            .await?;
        song_localized_title_history::Entity::insert_many(b)
            .exec(tx)
            .await?;
    }

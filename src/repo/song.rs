...
pub async fn create_many(
    ...
    let (
        mut correction_revision_active_models,
    ...
    .for_each(|(data, new_song, new_song_history, correction)| {
        correction_revision_active_models.push(
            correction_revision::ActiveModel {
                correction_id: Set(correction.id),
                entity_history_id: Set(new_song.id),
                description: Set(data.metadata.description.clone()),
            },
        );
    ...
    correction_revision::Entity::insert_many(correction_revision_active_models)
        .exec_with_returning_many(tx)
        .await?;
